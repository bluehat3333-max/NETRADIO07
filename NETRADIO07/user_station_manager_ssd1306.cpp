...
UserStationManager_SSD1306::UserStationManager_SSD1306(Adafruit_SSD1306* disp) {
    display = disp;
    server = new WebServer(80);
}

UserStationManager_SSD1306::~UserStationManager_SSD1306() {
    if (server) {
        stopConfigMode();
        delete server;
    }
...
bool UserStationManager_SSD1306::initSPIFFS() {
...
bool UserStationManager_SSD1306::loadStationsFromFile() {
...
void UserStationManager_SSD1306::saveStationsToFile() {
    DynamicJsonDocument doc(8192);
...
void UserStationManager_SSD1306::startConfigMode() {
    ...
    // Display config mode info on SSD1306
    ...
    display->println("Use web browser");
    display->println("Press button to exit");
    display->display();
}

void UserStationManager_SSD1306::stopConfigMode() {
    if (!web_server_running) return;
...
void UserStationManager_SSD1306::handleClient() {
    if (web_server_running) {
        server->handleClient();
    }
}

String UserStationManager_SSD1306::generateWebInterface() {
    String html = R"(
<!DOCTYPE html>
<html>
<head>
    ...
    <title>NETRADIO07 Station Manager</title>
    <style>
        ...
        .device-info { background: #e1f5fe; padding: 10px; border-radius: 5px; margin: 20px 0; text-align: center; }
    </style>
</head>
<body>
    <div class="container">
        <h1 class="header">📻 NETRADIO07 Station Manager</h1>
        <div class="device-info">
            <strong>Device:</strong> ESP32 Internet Radio with SSD1306 Display
        </div>
        ...
        function exportStations() {
            fetch('/stations')
                ...
                .then(data => {
                    ...
                    a.href = url;
                    a.download = 'netradio07_stations.json';
                    a.click();
...
void UserStationManager_SSD1306::handleRoot() {
    server->send(200, "text/html", generateWebInterface());
}

void UserStationManager_SSD1306::handleAddStation() {
...
void UserStationManager_SSD1306::handleDeleteStation() {
...
void UserStationManager_SSD1306::handleGetStations() {
    DynamicJsonDocument doc(8192);
    JsonArray stations = doc.createNestedArray("stations");
...
void UserStationManager_SSD1306::handleSaveStations() {
    saveStationsToFile();
    server->send(200, "text/plain", "Stations saved successfully!");
}

void UserStationManager_SSD1306::handleNotFound() {
    server->send(404, "text/plain", "Not found");
}

bool UserStationManager_SSD1306::addStation(const String& name, const String& url) {
...
bool UserStationManager_SSD1306::deleteStation(int index) {
...
void UserStationManager_SSD1306::clearStations() {
    station_names.clear();
    station_urls.clear();
    station_list_changed = true;
...
bool UserStationManager_SSD1306::importStationsFromUrl(const String& url) {
...
String UserStationManager_SSD1306::exportStationsToJson() {
    DynamicJsonDocument doc(8192);
    JsonArray stations = doc.createNestedArray("stations");
